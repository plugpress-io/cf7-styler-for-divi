import type { ModuleConversionOutline } from '@divi/types';

export const conversionOutline: ModuleConversionOutline = {
	advanced: {
		admin_label: 'module.meta.adminLabel',
		animation: 'module.decoration.animation',
		background: 'module.decoration.background',
		borders: {
			default: 'module.decoration.border',
		},
		box_shadow: {
			default: 'module.decoration.boxShadow',
		},
		disabled_on: 'module.decoration.disabledOn',
		filters: {
			default: 'module.decoration.filters',
		},
		height: 'module.decoration.sizing',
		link_options: 'module.advanced.link',
		margin_padding: 'module.decoration.spacing',
		max_width: 'module.decoration.sizing',
		module: 'module.advanced.htmlAttributes',
		overflow: 'module.decoration.overflow',
		position_fields: 'module.decoration.position',
		scroll: 'module.decoration.scroll',
		sticky: 'module.decoration.sticky',
		transform: 'module.decoration.transform',
		transition: 'module.decoration.transition',
		z_index: 'module.decoration.zIndex',
	},
	css: {
		before: 'css.*.before',
		main_element: 'css.*.mainElement',
		after: 'css.*.after',
		formContainer: 'css.*.formContainer',
		formFields: 'css.*.formFields',
		formLabels: 'css.*.formLabels',
		formInputs: 'css.*.formInputs',
		submitButton: 'css.*.submitButton',
	},
	module: {
		cf7: 'cf7.advanced.formId',
		use_form_header: 'cf7.advanced.useFormHeader',
		form_header_title: 'cf7.advanced.formHeaderTitle',
		form_header_text: 'cf7.advanced.formHeaderText',
		use_icon: 'cf7.advanced.useIcon',
		header_img: 'cf7.advanced.headerImage',
		header_icon: 'cf7.advanced.headerIcon',
		form_header_bg: 'cf7.advanced.formHeaderBg',
		form_header_padding: 'cf7.advanced.formHeaderPadding',
		form_header_bottom: 'cf7.advanced.formHeaderBottom',
		form_header_img_bg: 'cf7.advanced.formHeaderImgBg',
		form_header_icon_color: 'cf7.advanced.formHeaderIconColor',
		form_bg: 'cf7.advanced.formBg',
		form_padding: 'cf7.advanced.formPadding',
		use_form_button_fullwidth: 'cf7.advanced.useFormButtonFullwidth',
		button_alignment: 'cf7.advanced.buttonAlignment',
		form_field_height: 'cf7.advanced.formFieldHeight',
		form_field_padding: 'cf7.advanced.formFieldPadding',
		form_background_color: 'cf7.advanced.formBackgroundColor',
		form_field_active_color: 'cf7.advanced.formFieldActiveColor',
		form_field_spacing: 'cf7.advanced.formFieldSpacing',
		form_label_spacing: 'cf7.advanced.formLabelSpacing',
		cr_custom_styles: 'cf7.advanced.crCustomStyles',
		cr_size: 'cf7.advanced.crSize',
		cr_background_color: 'cf7.advanced.crBackgroundColor',
		cr_selected_color: 'cf7.advanced.crSelectedColor',
		cr_border_color: 'cf7.advanced.crBorderColor',
		cr_border_size: 'cf7.advanced.crBorderSize',
		cr_label_color: 'cf7.advanced.crLabelColor',
		cf7_message_padding: 'cf7.advanced.cf7MessagePadding',
		cf7_message_margin_top: 'cf7.advanced.cf7MessageMarginTop',
		cf7_message_alignment: 'cf7.advanced.cf7MessageAlignment',
		cf7_message_color: 'cf7.advanced.cf7MessageColor',
		cf7_message_bg_color: 'cf7.advanced.cf7MessageBgColor',
		cf7_border_highlight_color: 'cf7.advanced.cf7BorderHighlightColor',
		cf7_success_message_color: 'cf7.advanced.cf7SuccessMessageColor',
		cf7_success_message_bg_color: 'cf7.advanced.cf7SuccessMessageBgColor',
		cf7_success_border_color: 'cf7.advanced.cf7SuccessBorderColor',
		cf7_error_message_color: 'cf7.advanced.cf7ErrorMessageColor',
		cf7_error_message_bg_color: 'cf7.advanced.cf7ErrorMessageBgColor',
		cf7_error_border_color: 'cf7.advanced.cf7ErrorBorderColor',

		// Field Font (D4: form_field_font)
		form_field_font_size: 'cf7.advanced.fieldFontSize',
		form_field_font_weight: 'cf7.advanced.fieldFontWeight',
		form_field_line_height: 'cf7.advanced.fieldLineHeight',
		form_field_letter_spacing: 'cf7.advanced.fieldLetterSpacing',
		form_field_text_transform: 'cf7.advanced.fieldTextTransform',

		// Label Font (D4: labels)
		labels_font_size: 'cf7.advanced.labelFontSize',
		labels_font_weight: 'cf7.advanced.labelFontWeight',
		labels_line_height: 'cf7.advanced.labelLineHeight',
		labels_letter_spacing: 'cf7.advanced.labelLetterSpacing',
		labels_text_transform: 'cf7.advanced.labelTextTransform',
		labels_text_color: 'cf7.advanced.formLabelColor',

		// Placeholder Color
		placeholder_text_color: 'cf7.advanced.placeholderColor',

		// Title Font (D4: title)
		title_font_size: 'cf7.advanced.headerTitleFontSize',
		title_font_weight: 'cf7.advanced.headerTitleFontWeight',
		title_line_height: 'cf7.advanced.headerTitleLineHeight',
		title_letter_spacing: 'cf7.advanced.headerTitleLetterSpacing',
		title_text_transform: 'cf7.advanced.headerTitleTextTransform',
		title_text_color: 'cf7.advanced.headerTitleTextColor',

		// Text Font (D4: text)
		text_font_size: 'cf7.advanced.headerTextFontSize',
		text_font_weight: 'cf7.advanced.headerTextFontWeight',
		text_line_height: 'cf7.advanced.headerTextLineHeight',
		text_letter_spacing: 'cf7.advanced.headerTextLetterSpacing',
		text_text_transform: 'cf7.advanced.headerTextTextTransform',
		text_text_color: 'cf7.advanced.headerTextTextColor',

		// Button Font (D4: submit_button)
		submit_button_font_size: 'cf7.advanced.buttonFontSize',
		submit_button_font_weight: 'cf7.advanced.buttonFontWeight',
		submit_button_line_height: 'cf7.advanced.buttonLineHeight',
		submit_button_letter_spacing: 'cf7.advanced.buttonLetterSpacing',
		submit_button_text_transform: 'cf7.advanced.buttonTextTransform',
	},
};
