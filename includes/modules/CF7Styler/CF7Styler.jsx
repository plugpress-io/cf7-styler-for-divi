...
class CF7_Styler extends Component {
  ...
  static css(props) {
    ...
    let form_header_padding_responsive_status = form_header_padding_last_edited && form_header_padding_last_edited.startsWith("on");
    ...
    let form_padding_responsive_status = form_padding_last_edited && form_padding_last_edited.startsWith("on");
    ...
    additionalCss.push([{
        selector: "%%order_class%% input:not([type=submit]), %%order_class%% select, %%order_class%% .dipe-cf7 textarea, %%order_class%% .wpcf7-checkbox input[type=checkbox] + span:before, %%order_class%% .wpcf7-acceptance input[type=checkbox] + span:before, %%order_class%% .wpcf7-radio input[type=radio]:not(:checked) + span:before",
    ...
    _renderHeader(props) {
        ...
        //Icon & Image
        const header_img  = typeof props.header_img !== 'undefined' ? props.header_img : false;
        const image       = header_img ? <div className='dipe-form-header-image'><img src={header_img} alt=""/></div> : '';
        
        const header_icon = utils.processFontIcon( props.header_icon );
        const icon        = <div className="dipe-form-header-icon"><span className="et-pb-icon">{header_icon}</span></div>;
        const icon_image  = 'on' === props.use_icon ? icon : image;
       
        // Header
        const title = typeof props.form_header_title !== 'undefined' ? <h2 className="dipe-form-header-title">{props.form_header_title}</h2> : '';
        const text  = typeof props.form_header_text !== 'undefined' ? <div className="dipe-form-header-text">{props.form_header_text}</div> : '';
        
        const header_info  = title || text ? <div className="dipe-form-header-info">{title} {text}</div> : '';
    ...
    render() {
        ...
        return (
            <div className={`dipe-cf7-container dipe-cf7-button-${button_alignment}`}>
                {this._renderHeader(props)}
                <div className={`dipe-cf7 dipe-cf7-styler ${cr_custom_class}`} dangerouslySetInnerHTML={{ __html: props.__cf7form }} />
            </div>
        );
    }
